...
<html lang="en">
  ...
  <body>
    ...
    @php
        $sectionIds = ['banner', 'gasification', 'metal', 'catalyst', 'battery-manufacturing'];
    @endphp

    <div id="fullpage">
        @foreach($homeDetails as $index => $home)
            @php
                $sectionId = $sectionIds[$index] ?? 'unknown-section';

                // Decode JSON data for individual card fields
                $cardTitles = json_decode($home->card_title, true) ?? [];
                $companyLogos = json_decode($home->company_logo, true) ?? [];
                ...
                $cards = [];
                $totalCards = max(count($cardTitles), count($companyLogos), count($companyNames), count($descriptions));

                for ($i = 0; $i < $totalCards; $i++) {
                    $cards[] = [
                        'card_title' => $cardTitles[$i] ?? null,
                        'company_logo' => $companyLogos[$i] ?? null,
                        'company_name' => $companyNames[$i] ?? null,
